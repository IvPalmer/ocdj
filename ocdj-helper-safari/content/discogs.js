// OCDJ Standalone — Discogs Content Script
// Injects Queue + Wantlist buttons on release, label, marketplace, and search pages
...
(() => {
  ...
  function injectReleaseHeader(meta, releaseId) {
    ...
    // Queue release button (fetches Discogs API for YouTube videos)
    if (releaseId) {
    ...
    buttons.push(OCDJ.createDigButton({
      size: 'medium',
      tooltip: `Add "${meta.artist || ''} - ${meta.release_name || ''}" to Wantlist`,
