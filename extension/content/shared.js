// OCDJ Dig — Shared Content Script Utilities
...
const OCDJ = (() => {
  ...
  function createDigButton({ onClick, tooltip = 'Add to Wanted List', size = 'small', label }) {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (result && result.duplicate) {
          markAsDuplicate(btn);
          showToast(`Already in Wanted List (${Math.round(result.fuzzy_score)}% match)`, 'duplicate');
        } else if (result && result.created) {
          markAsAdded(btn);
          const artist = result.item?.artist || '';
          const title = result.item?.title || '';
          showToast(`Added: ${artist}${artist && title ? ' - ' : ''}${title}`, 'success');
  ...
  function markAsAdded(btn) {
    ...
    btn.textContent = '\u2713';
    btn.title = 'Added to Wanted List';
  }
  ...
  function markAsDuplicate(btn) {
    ...
    btn.textContent = '\u2713';
    btn.title = 'Already in Wanted List';
  }
