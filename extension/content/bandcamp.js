// OCDJ Dig — Bandcamp Content Script
...
(() => {
  ...
  function extractTralbumData() {
    // Bandcamp embeds a data-tralbum JSON blob on album/track pages
  ...
  function extractMeta() {
    ...
    if (tralbum) {
      ...
      // Label from band info or artist name
      if (tralbum.current?.band_id !== tralbum.current?.selling_band_id) {
        // This is on a label's page — artist is in tralbum.artist, label is the site
  ...
  function injectAlbumTrackPage() {
    ...
    if (buyArea && !OCDJ.isInjected(buyArea)) {
      ...
      const bcEmbedUrl = tralbumId
        ...
        : '';
      const bcThumb = document.querySelector('meta[property="og:image"]')?.content || '';
      ...
      const queueBtn = OCDJ.createPlayButton({
        ...
        onClick: () => {
          ...
          // Extract artwork at click time (more reliable on slow loads)
          const thumb = document.querySelector('meta[property="og:image"]')?.content ||
                        document.querySelector('#tralbumArt img')?.src || bcThumb;
      ...
      const btn = OCDJ.createDigButton({
        size: 'large',
        tooltip: 'Add to OCDJ Wanted List',
    ...
    if (meta.tracks && meta.tracks.length > 1) {
      ...
      trackRows.forEach((row, i) => {
        ...
        buttons.push(OCDJ.createDigButton({
          size: 'small',
          tooltip: `Add "${meta.artist} - ${trackTitle}" to Wanted List`,
          onClick: () => OCDJ.sendToBackground('dig:add', {
            artist: meta.artist || '',
            title: trackTitle,
            release_name: meta.release_name || '',
  ...
  function injectLabelPage() {
    ...
    albumCards.forEach((card) => {
      ...
      const btn = OCDJ.createDigButton({
        size: 'small',
        tooltip: `Add "${artist ? artist + ' - ' : ''}${title}" to Wanted List`,
        onClick: () => OCDJ.sendToBackground('dig:add', {
          artist,
          title: '',
          release_name: title,
