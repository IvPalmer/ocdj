...
class ClaudeVisionCollector:
    ...
    async def identify_album_with_web(self, image: Image.Image, hint: Optional[Dict] = None, timeout_seconds: int = 180) -> Dict:
        ...
        Triggered by hybrid_search when Pass 1 returns null artist (e.g.
        generic-label promo sleeves like Safe-In-Sound, or covers where
        Claude only reads a title with no artist visible). Pass 2 enables
        the WebSearch tool so Opus can resolve the specific release by
        searching for the label catalog + visible text (the same path
        codex/GPT-5 took to identify the user's u3 record as
        'Tony Senghore & DJ Jonene - Dem Bitz' on Safe In Sound SISM 001).

        Multi-turn (max_turns=4) so Opus has room for: search → read result
        → search again if needed → answer. Costlier per call but only fires
        on the hard tail.
        """
        if not self.configured:
            return {'success': False, 'error': 'CLAUDE_CODE_OAUTH_TOKEN not configured'}
        ...
        if hint:
            bits = []
            if hint.get('label'): bits.append(f"label visible on sleeve: {hint['label']}")
            if hint.get('album'): bits.append(f"album/track title visible: {hint['album']}")
            if hint.get('artist'): bits.append(f"possible artist: {hint['artist']}")
            if hint.get('evidence'): bits.append(f"first-pass evidence: {hint['evidence']}")
            if bits:
                hint_text = '\n\nFirst-pass read from this cover:\n  - ' + '\n  - '.join(bits)

        prompt_text = (
            f"This is a 12\" vinyl record. The sleeve is generic or limited "
            f"on visible text, so the FIRST pass couldn't pin a specific "
            f"artist. Use the WebSearch tool to identify the actual release."
            f"{hint_text}\n\n"
            f"Strategy:\n"
            f"1. Look at the disc center label visible through the sleeve — it "
            f"   often has the artist, track titles, catalog number, even "
            f"   rotated/upside-down text.\n"
            f"2. Search the web for the label name + any visible text (catalog "
            f"   number, track title, partial artist name).\n"
            f"3. Verify the match by cross-checking what's visible on the "
            f"   sleeve/disc against the candidate release.\n\n"
            f"Return ONLY a JSON object (no prose, no fences):\n"
            f'{{ "artist": str|null, "album": str|null, "label": str|null, '
            f'"confidence": "high|medium|low", '
            f'"evidence": "concise note + the source URL you verified against" }}\n\n'
            f"Same anti-hallucination rule: if you can't verify a specific "
            f"release, return null for artist/album, keep the label, low "
            f"confidence. Don't guess from genre/style."
        )
        ...
        async def _gen():
            yield {
                ...
                'parent_tool_use_id': None,
                'session_id': 'cratemate-identify-web',
            }
        ...
        try:
            ...
            logger.error('claude_vision (pass2) SDK error: %s', e, exc_info=True)
            return {'success': False, 'error': f'pass2 sdk: {e}'}

        raw = '\n'.join(collected).strip()
        parsed = self._parse_response(raw)
        if parsed is None:
            return {'success': False, 'error': 'pass2 unparseable', 'raw_response': raw}
        ...
        logger.info(
            'claude_vision (pass2): %r / %r (label=%r, conf=%s)',
            result['artist'], result['album'], result['label'], result['confidence'],
        )
        return {'success': True, 'result': result, 'raw_response': raw}
