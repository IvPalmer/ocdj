...
load_dotenv()
# CRATEMATE_-prefixed envs to avoid colliding with ocdj's own SPOTIFY_CLIENT_ID
# (used by the wanted/recognize modules for a different OAuth flow).
CLIENT_ID = os.getenv("CRATEMATE_SPOTIFY_CLIENT_ID") or os.getenv("SPOTIFY_CLIENT_ID")
CLIENT_SECRET = os.getenv("CRATEMATE_SPOTIFY_CLIENT_SECRET") or os.getenv("SPOTIFY_CLIENT_SECRET")
...
class SpotifyCollector(MetadataCollector):
    ...
    def __init__(self, name: str):
        ...
        if not CLIENT_ID or CLIENT_ID == '__PENDING__' or not CLIENT_SECRET or CLIENT_SECRET == '__PENDING__':
            self.logger.warning(
                "SpotifyCollector unconfigured (CRATEMATE_SPOTIFY_CLIENT_ID/SECRET missing); "
                "fetch_* will return empty results."
            )
            return
        ...
        try:
            ...
            self.logger.warning("Spotify client init failed: %s", e)
            self.client = None
